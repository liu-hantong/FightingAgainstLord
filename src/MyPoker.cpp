...
void MyPoker::Strategy_6()
{
	...
	for (flag = 0; flag < MyNumber - (EnemyNumber - 1); ++flag)
	{
		...
		for (; EqualExaminePosition < (int)result.size() - 3; EqualExaminePosition += 2)
		{
...
void MyPoker::Strategy_7()
{
	...
	for (flag = 0; flag < MyNumber - (EnemyNumber - 1); ++flag)
	{
		...
		for (int SeriesFlag = 0; SeriesFlag < ((int)PokerSeries.size() - (EnemyNumber + 1)); ++SeriesFlag)
		{
	...
	for (flag = 0; flag < MyNumber - 5; ++flag)
	{
		...
		else
		{
			...
			if (IfAllEquall(formerPart) && IfAllEquall(latterPart))
			{
				...
				int FirstWingFlag = 0;
				for (; FirstWingFlag < (int)Remain.size() - 1; ++FirstWingFlag)
				{
				...
				for (; SecondWingFlag < (int)Remain2.size() - 1; ++SecondWingFlag)
				{
